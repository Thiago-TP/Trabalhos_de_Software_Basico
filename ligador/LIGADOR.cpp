...
void begin_exc_and_get_offsets(const char* objfile_name) {
...
void fill_global_tables_and_exc(const char* objfile_name, int file_index) {
    ifstream input_file (objfile_name);
...
int main (int argc, char *argv[]) {
    for(int i=1; i<argc; i++) {
        begin_exc_and_get_offsets(argv[i]);
        fill_global_tables_and_exc(argv[i], i-1); 
    }
    make_struct_vector(); 
    finish_executable();
